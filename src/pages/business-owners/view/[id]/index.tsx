...
function BusinessOwnerViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessOwnerInterface>(
    ...
    () =>
      getBusinessOwnerById(id, {
        relations: ['user', 'game', 'team_member'],
      }),
  );
  ...
  const team_memberHandleCreate = async () => {
    setCreateError(null);
    try {
      await createTeamMember({ business_owner_id: id, user_id: team_memberUserId });
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Business Owner Detail View
          </Text>
          {hasAccess('business_owner', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/business-owners/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('game', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/games/create?business_owner_id=${data?.id}`}>
