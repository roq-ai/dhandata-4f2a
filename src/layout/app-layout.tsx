...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<PlayerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/players` : null),
    () => getPlayers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Player',
      path: isTenantUser ? `/players/view/${data?.[0]?.id}` : '/players',
      entity: 'player',
    ...
    { name: 'Win', path: '/wins', entity: 'win', service: AccessServiceEnum.PROJECT, icon: FiBox },
